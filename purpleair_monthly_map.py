#!/usr/bin/env python3
"""
generated by Anthropic/Claude AI
PurpleAir Monthly Average Map Generator

Creates a single static map showing the monthly average AQI at each sensor.
Perfect for website display or reports.
...
Author: Generated for Golden BC webmaster
Date: 2025
"""
...
def fetch_monthly_data(api_key, sensors_df, year, month, output_csv):
    """
    Fetch hourly data for entire month for all sensors.
...
def calculate_monthly_averages(sensors_df, historical_df):
    """Calculate monthly average PM2.5 for each sensor."""
    print("\nCalculating monthly averages...")
    
    # Group by sensor and calculate mean
    monthly_avg = historical_df.groupby('sensor_index')['pm2.5'].agg([
        ('avg_pm25', 'mean'),
        ('min_pm25', 'min'),
        ('max_pm25', 'max'),
        ('count', 'count')
    ...
    print(f"  Calculated averages for {len(result[result['avg_pm25'].notna()])} sensors")
...
def create_monthly_map(sensors_avg_df, bbox, region_name, month_str, output_path, 
    ...
    if len(valid_aqi) > 0:
        stats_text = (
            ...
            f"Data points per sensor: {valid_sensors['count'].mean():.0f} avg"
        )
        ax.text(
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate monthly average AQI map for PurpleAir sensors',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate January 2026 map
  python3 purpleair_monthly_map.py --month 2026-01 --background golden.tif --output jan2026.png
  
  # Use cached data (no API calls)
  python3 purpleair_monthly_map.py --month 2026-01 --background golden.tif --output jan2026.png --use-cached
  
  # Different region (Vancouver)
  python3 purpleair_monthly_map.py --month 2026-01 --region vancouver --output vancouver_jan.png
        """
    )
    ...
    data_csv = os.path.join(args.data_dir, f"{args.region}_{year}-{month:02d}_data.csv")
